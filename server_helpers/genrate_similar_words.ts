...
export function generateSimilarWords(search_query: string): Set<string> {
    ...
    for (const word of words) {
        ...
        if (word.length >= 3) similarWords.add(word);
        ...
        if (arabicConverted.length >= 3) similarWords.add(arabicConverted);
        if (englishConverted.length >= 3) similarWords.add(englishConverted);
        ...
        if (!baseWord.startsWith("ال") && baseWord.length > 1) {
            ...
            if (withAl.length >= 3) similarWords.add(withAl);
        }

        // إضافة الكلمة بدون "ال" التعريف إذا كانت موجودة
        if (baseWord.startsWith("ال") && baseWord.length > 1) {
            const withoutAl = baseWord.substring(2);
            if (withoutAl.length >= 3) similarWords.add(withoutAl);
        }

        // إضافة الكلمة الأساسية بعد إزالة "ة" أو "ه"
        if (baseWord.length >= 3) similarWords.add(baseWord);
        ...
        for (let i = 0; i < word.length; i++) {
            ...
            if (similarChars[char]) {
                for (const similarChar of similarChars[char]) {
                    ...
                    if (similarWord.length >= 3) similarWords.add(similarWord);
                }
            }
        }
    }
